...
use ferrl::{
    evaluate, load_adapter, load_qwen_policy_bound, save_adapter, HfTokenizer, LoaderOpts, Policy,
    QwenGradModel, QwenPolicy, RewardError, RewardFn, RunDir, Sample, TokenizerLike, Trainer,
    TrainerConfig,
};
...
fn qwen_v4_resume_smoke_on_gpu() {
    ...
    let (mut policy, tok, checkpoint_policy_sha256) =
        load_qwen_policy_bound(&dir, &device, &loader_opts)
            .expect("load integrity-bound Qwen policy");
    let samples = vec![Sample::new("The capital of France is", ())];
    ...
    let resume_opts = LoaderOpts {
        seed: 9999,
        ..loader_opts
    };
    let (mut policy2, _resume_tok, resume_policy_sha256) =
        load_qwen_policy_bound(&dir, &device, &resume_opts)
            .expect("load fresh integrity-bound resume policy");
    assert_eq!(
        resume_policy_sha256, checkpoint_policy_sha256,
        "sampler seed must not split one immutable policy identity"
    );

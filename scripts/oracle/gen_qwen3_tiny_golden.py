#!/usr/bin/env python3
"""Build and dump the committed tiny native-Qwen3 forward oracle.

The checkpoint and logits come from the official Transformers implementation
(`Qwen3ForCausalLM`, transformers 5.11.0, torch 2.12.0, fp32 CPU).  The tiny
shape deliberately exercises Qwen's explicit attention head dimension, GQA,
per-head Q/K normalization, and RoPE at every sequence position.

Transformers 5.11 writes the authoritative RoPE settings under
``rope_parameters``.  Candle's Qwen3 serde mirror still consumes the legacy
top-level ``rope_theta`` scalar, so this generator retains the upstream block
and adds an equal scalar compatibility mirror.  The Rust gate asserts both the
bridge and its provenance; it cannot silently test candle's default theta.

Outputs (committed under ``crates/ferrl/tests/fixtures/tiny_qwen3/``):
...
Pinned regeneration command::
    ...
    conda activate ferrl-oracle && python scripts/oracle/gen_qwen3_tiny_golden.py
"""
...
OUT_DIR = (
    pathlib.Path(__file__).resolve().parents[2] / "crates/ferrl/tests/fixtures/tiny_qwen3"
)
SEED = 31415
WEIGHT_STD = 0.45
...
ROPE_BRIDGE = "upstream rope_parameters.rope_theta mirrored to candle rope_theta"
...
def tiny_config() -> Qwen3Config:
    return Qwen3Config(
        ...
        # Attention width 4 * 8 = 32 intentionally differs from hidden size.
        head_dim=8,
        hidden_act="silu",
...
def install_candle_rope_bridge() -> None:
    ...
    assert isinstance(rope_parameters, dict), "upstream config omitted rope_parameters"
    assert rope_parameters.get("rope_type") == "default", rope_parameters
    assert rope_parameters.get("rope_theta") == ROPE_THETA, rope_parameters
...
def main() -> None:
    ...
    golden = {
        "meta": {
            ...
            "reference": "transformers.Qwen3ForCausalLM/Qwen3Config",

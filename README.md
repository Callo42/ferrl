...
Because ferrl no longer owns autodiff, correctness is pinned against independent
references:

1. **GRPO math vs TRL / DeepSeekMath** — a committed golden JSON
   (`crates/ferrl/tests/fixtures/grpo_golden.json`) generated by a dependency-free
   Python oracle (`scripts/gen_golden.py`). The Rust test
   `grpo::tests::matches_golden_fixture` loads it and asserts agreement. See
   [Oracle](#oracle--golden-fixture).
2. **Model forward vs candle's shipped forward** (added with the model layer).
3. **Grad-coverage canary** — after `backward`, every trainable LoRA `Var` must have a
   `Some` and non-zero gradient (`lora::tests::grad_coverage_canary_every_var_gets_nonzero_grad`).
   This guards a real footgun: candle optimizers **silently skip** any parameter
   missing from the `GradStore`.
4. **End-to-end finite-difference gradcheck** of the loss w.r.t. the LoRA params
   (added with the trainer).
...
math itself changes. The oracle has **no third-party dependencies** (pure stdlib), so
...
Formulas (per token, with `std` the *population* / ddof=0 standard deviation):
